...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  useEffect(() => {
    ...
    globeRef.current
      ...
      .arcStroke((e: any) => {
        // Use arc order for deterministic stroke width
        const strokeIndex = ((e as { order: number }).order - 1) % 3;
        return [0.32, 0.28, 0.3][strokeIndex];
      })
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      for (let i = 0; i < selectionCount; i++) {
        selectedIndices.push((cyclePosition + i) % data.length);
      }
